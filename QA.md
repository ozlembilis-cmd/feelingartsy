...
- The local Mistral key authenticated successfully and the owner confirmed Free mode with pay-as-you-go disabled. Synthetic generation requests returned HTTP 429, including a minimal diagnostic request. Mistral's status page reported its free API temporarily disabled on 7 September. Production remains unconfigured; successful generation and model quality are unverified. The app continues to show a truthful local summary.
