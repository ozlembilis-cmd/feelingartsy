...
Create my art profile is available from the intro, palette icon, and collection drawer. Five quiet dots appear only inside the panel. Distinct saved or responded-to artworks count once; height changes and artist guesses do not count. Five is a suggested target, never a required quiz. A local first-impressions summary is clearly labelled as non-AI. The optional Mistral adapter is prepared, but live generation remains unavailable until a free account is connected and verified. No live model calls have been made.
